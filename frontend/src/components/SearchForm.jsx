...
const SearchForm = () => {
    ...
    <Toaster position="top-center" reverseOrder={false} />
        ...
        <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-4 gap-3 items-center mt-6">
            <Input onChange={(e)=>setFormInput(prev=> ({...prev,from:e.target.value}))} name="from" label={"From"} placeholder="Departure City">
            <FaLocationDot className="left-1 absolute top-1/2 transform -translate-y-1/2 " />
            </Input>
            <Input name="to" onChange={(e)=>setFormInput(prev=> ({...prev,from:e.target.value}))}  label={"To"} placeholder="Arrival City">
            <FaLocationDot className="left-1 absolute top-1/2 transform -translate-y-1/2 " />
            </Input>
            <Input onChange={(e)=>setFormInput(prev=> ({...prev,from:e.target.value}))} name="date" type="date" label={"Date"} placeholder="Arrival City">
            </Input>
            <button className="flex mt-5 gap-1 font-semibold items-center bg-accent px-6 border border-stone-400 py-[6px] rounded-md "><FaSearch /><span>Search</span></button>
